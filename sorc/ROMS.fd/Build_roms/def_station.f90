...
!
!svn $Id: def_station.F 1122 2022-04-13 19:50:43Z arango $
!================================================== Hernan G. Arango ===
...
!
      SELECT CASE (STA(ng)%IOtype)
        CASE (io_nf90)
          CALL def_station_nf90 (ng, ldef)
        CASE DEFAULT
          IF (Master) THEN
            WRITE (stdout,10) STA(ng)%IOtype
  10        FORMAT (' DEF_STATION - Illegal output type, io_type = ',i0)
          END IF
          exit_flag=3
...
!
!***********************************************************************
      SUBROUTINE def_station_nf90 (ng, ldef)
!***********************************************************************
!
      USE mod_netcdf
!
...
!
     ...
     &                   SetFillVal = .TRUE.,                           &
     &                   SetParAccess = .TRUE.)
          IF (FoundError(exit_flag, NoError, 401, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     SetFillVal = .TRUE.,                         &
     &                     SetParAccess = .TRUE.)
            IF (FoundError(exit_flag, NoError, 499, MyFile)) RETURN
          END IF
        END DO
!
...
!
     ...
     &                   SetFillVal = .TRUE.,                           &
     &                   SetParAccess = .TRUE.)
          IF (FoundError(exit_flag, NoError, 580, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   SetFillVal = .TRUE.,                           &
     &                   SetParAccess = .TRUE.)
          IF (FoundError(exit_flag, NoError, 806, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   SetFillVal = .TRUE.,                           &
     &                   SetParAccess = .TRUE.)
          IF (FoundError(exit_flag, NoError, 824, MyFile)) RETURN
        END IF
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1533, MyFile)) THEN
...
!
     ...
     &                         ncid = STA(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1542, MyFile)) RETURN
!
