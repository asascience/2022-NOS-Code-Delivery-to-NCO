...
!
!svn $Id: get_nudgcoef.F 1099 2022-01-06 21:01:01Z arango $
!================================================== Hernan G. Arango ===
...
!
  10  FORMAT (' GET_NUDGCOEF - Illegal output type, io_type = ',i0)
!
      RETURN
...
!
     ...
     &                    GRID(ng) % rmask,                             &
     &                    CLIMA(ng) % M3nudgcof)
        IF (FoundError(status, nf90_noerr, 343, MyFile)) THEN
          exit_flag=2
          ioerror=status
          RETURN
        ELSE
...
!
     ...
     &                         nVarDim = nvdim,                         &
     &                         nVarAtt = nvatt)
          IF (FoundError(exit_flag, NoError, 397, MyFile)) RETURN
          DO i=1,nvatt
            IF (TRIM(var_Aname(i)).eq.'units') THEN
              tunits=TRIM(var_Achar(i))
              IF (tunits(1:3).eq.'day') THEN
...
!
     ...
     &                      CLIMA(ng) % Tnudgcof(:,:,:,ic))
          IF (FoundError(status, nf90_noerr, 423, MyFile)) THEN
            exit_flag=2
            ioerror=status
            RETURN
          ELSE
