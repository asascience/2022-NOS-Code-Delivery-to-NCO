...
!
!svn $Id: wrt_station.F 1099 2022-01-06 21:01:01Z arango $
!================================================== Hernan G. Arango ===
...
!
      SELECT CASE (STA(ng)%IOtype)
        CASE (io_nf90)
          CALL wrt_station_nf90 (ng, tile,                              &
     &                           LBi, UBi, LBj, UBj)
        CASE DEFAULT
          IF (Master) THEN
            WRITE (stdout,10) STA(ng)%IOtype
  10        FORMAT (' WRT_STATION - Illegal output type, io_type = ',i0)
          END IF
          exit_flag=3
      END SELECT
...
!
!***********************************************************************
      SUBROUTINE wrt_station_nf90 (ng, tile,                            &
     &                             LBi, UBi, LBj, UBj)
!***********************************************************************
!
      USE mod_netcdf
...
!
     ...
     &                        varid = STA(ng)%Vid(idv3dN))
        IF (FoundError(exit_flag, NoError, 410, MyFile)) RETURN
        deallocate (Ur3d)
        deallocate (Vr3d)
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idDano))
        IF (FoundError(exit_flag, NoError, 484, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idVvis))
        IF (FoundError(exit_flag, NoError, 538, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idMtls))
        IF (FoundError(exit_flag, NoError, 610, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idEmPf))
        IF (FoundError(exit_flag, NoError, 785, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idevap))
        IF (FoundError(exit_flag, NoError, 801, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                        varid = STA(ng)%Vid(idVbms))
        IF (FoundError(exit_flag, NoError, 883, MyFile)) RETURN
      END IF
!
!-----------------------------------------------------------------------
