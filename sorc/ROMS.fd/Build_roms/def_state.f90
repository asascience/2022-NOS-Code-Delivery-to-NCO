...
!
      SELECT CASE (S(ng)%IOtype)
        ...
        CASE DEFAULT
          IF (Master) THEN
            WRITE (stdout,10) AVG(ng)%IOtype
  10        FORMAT (' DEF_STATE - Illegal output type, io_type = ',i0)
          END IF
          exit_flag=3
      END SELECT
      IF (FoundError(exit_flag, NoError, 94, MyFile)) RETURN
!
      RETURN
...
!
      DEFINE : IF (Lcreate) THEN
        ...
        IF (FoundError(exit_flag, NoError, 178, MyFile)) THEN
...
!
        ...
        IF (FoundError(exit_flag, NoError, 487, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
     ...
     &                 SetParAccess = .TRUE.)
        IF (FoundError(exit_flag, NoError, 504, MyFile)) RETURN
!
!  Define free-surface.
!
...
!
     ...
     &                 NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 558, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FOUT, nvd3, v2dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 595, MyFile)) RETURN
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 820, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 851, MyFile)) THEN
...
!
     ...
     &                         ncid = S(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 860, MyFile)) RETURN
!
