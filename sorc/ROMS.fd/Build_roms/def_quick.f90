...
!
!svn $Id: def_quick.F 1122 2022-04-13 19:50:43Z arango $
!================================================== Hernan G. Arango ===
...
!
      SELECT CASE (QCK(ng)%IOtype)
        CASE (io_nf90)
          CALL def_quick_nf90 (ng, ldef)
        CASE DEFAULT
          IF (Master) THEN
            WRITE (stdout,10) QCK(ng)%IOtype
  10        FORMAT (' DEF_QUICK - Illegal output type, io_type = ',i0)
          END IF
          exit_flag=3
      END SELECT
...
!
!***********************************************************************
      SUBROUTINE def_quick_nf90 (ng, ldef)
!***********************************************************************
!
      USE mod_netcdf
!
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 571, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 591, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 611, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 847, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 885, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 950, MyFile)) RETURN
          END IF
        END DO
!
...
!
     ...
     &                     NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 979, MyFile)) RETURN
          END IF
        END DO
!
...
!
     ...
     &                   NF_FOUT, nvd4, w3dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 1102, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, w3dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 1144, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1298, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1426, MyFile)) RETURN
        END IF
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2284, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2291, MyFile)) RETURN
      END IF DEFINE
!
!=======================================================================
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2305, MyFile)) THEN
...
!
     ...
     &                         ncid = QCK(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 2314, MyFile)) RETURN
!
!  Inquire about the variables.
!
        CALL netcdf_inq_var (ng, iNLM, ncname,                          &
     &                       ncid = QCK(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 2320, MyFile)) RETURN
!
