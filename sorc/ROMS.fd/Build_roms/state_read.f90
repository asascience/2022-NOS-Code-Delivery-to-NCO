...
!
!svn $Id: state_read.F 1099 2022-01-06 21:01:01Z arango $
!================================================== Hernan G. Arango ===
...
!
  10  FORMAT (' STATE_READ - Illegal output type, io_type = ',i0)
!
      RETURN
...
!
     ...
     &                      my_ncid, (/rec/), (/1/))
      IF (FoundError(exit_flag, NoError, 482, MyFile)) RETURN
!
!  Report information.
!
      IF (Master) THEN
...
!
     ...
     &                  umask,                                          &
     &                  s_u(:,:,:,Lout))
      IF (FoundError(status, nf90_noerr, 765, MyFile)) THEN
        IF (Master) THEN
...
!
     ...
     &                    rmask,                                        &
     &                    s_t(:,:,:,Lout,itrc))
        IF (FoundError(status, nf90_noerr, 892, MyFile)) THEN
          IF (Master) THEN
...
!
      CALL wclock_off (ng, model, 80, 992, MyFile)
!
