...
!
!svn $Id: wrt_quick.F 1099 2022-01-06 21:01:01Z arango $
!================================================== Hernan G. Arango ===
...
!
  ...
  10        FORMAT (' WRT_QUICK - Illegal output type, io_type = ',i0)
          END IF
          exit_flag=3
      END SELECT
...
!
     ...
     &                     GRID(ng) % vmask,                            &
     &                     GRID(ng) % z_v)
        IF (FoundError(status, nf90_noerr, 359, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     OCEAN(ng) % ubar(:,:,kstp(ng)))
        IF (FoundError(status, nf90_noerr, 431, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     OCEAN(ng) % vbar(:,:,kstp(ng)))
        IF (FoundError(status, nf90_noerr, 453, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask_full,                       &
     &                     Vr2d)
        IF (FoundError(status, nf90_noerr, 511, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % vmask_full,                       &
     &                     OCEAN(ng) % v(:,:,:,nrhs(ng)))
        IF (FoundError(status, nf90_noerr, 559, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                       GRID(ng) % rmask_full,                     &
     &                       Ur3d)
          IF (FoundError(status, nf90_noerr, 648, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     Wr3d)
        IF (FoundError(status, nf90_noerr, 740, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     OCEAN(ng) % wvel)
        IF (FoundError(status, nf90_noerr, 763, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akv,                            &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 907, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % Tair)
        IF (FoundError(status, nf90_noerr, 1051, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % Vwind)
        IF (FoundError(status, nf90_noerr, 1095, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % lhflx)
        IF (FoundError(status, nf90_noerr, 1154, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % rain)
        IF (FoundError(status, nf90_noerr, 1243, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     FORCES(ng) % stflux(:,:,isalt))
        IF (FoundError(status, nf90_noerr, 1267, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % srflx)
        IF (FoundError(status, nf90_noerr, 1290, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % umask,                            &
     &                     FORCES(ng) % sustr)
        IF (FoundError(status, nf90_noerr, 1314, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % umask,                            &
     &                     FORCES(ng) % bustr)
        IF (FoundError(status, nf90_noerr, 1358, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % vmask,                            &
     &                     FORCES(ng) % bvstr)
        IF (FoundError(status, nf90_noerr, 1380, MyFile)) THEN
          IF (Master) THEN
...
!
      ...
      IF (FoundError(exit_flag, NoError, 2305, MyFile)) RETURN
