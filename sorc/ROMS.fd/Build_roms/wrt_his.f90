      MODULE wrt_his_mod
!
!svn $Id: wrt_his.F 1099 2022-01-06 21:01:01Z arango $
!================================================== Hernan G. Arango ===
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     GRID(ng) % z_r)
        IF (FoundError(status, nf90_noerr, 327, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % umask,                            &
     &                     GRID(ng) % z_v)
        IF (FoundError(status, nf90_noerr, 357, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     GRID(ng) % z_w)
        IF (FoundError(status, nf90_noerr, 409, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     OCEAN(ng) % vbar(:,:,kstp(ng)))
        IF (FoundError(status, nf90_noerr, 614, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask_full,                       &
     &                     Ur3d)
        IF (FoundError(status, nf90_noerr, 936, MyFile)) THEN
          IF (Master) THEN
     ...
     &                     GRID(ng) % rmask_full,                       &
     &                     Vr3d)
        IF (FoundError(status, nf90_noerr, 952, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     OCEAN(ng) % wvel)
        IF (FoundError(status, nf90_noerr, 1008, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akv,                            &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1181, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akt(:,:,:,itemp),               &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1204, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akt(:,:,:,isalt),               &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1228, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1295, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % Pair)
        IF (FoundError(status, nf90_noerr, 1357, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % Tair)
        IF (FoundError(status, nf90_noerr, 1381, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                       FORCES(ng) % stflx(:,:,itrc))
          IF (FoundError(status, nf90_noerr, 1458, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % lhflx)
        IF (FoundError(status, nf90_noerr, 1484, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % shflx)
        IF (FoundError(status, nf90_noerr, 1506, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % evap)
        IF (FoundError(status, nf90_noerr, 1551, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % rain)
        IF (FoundError(status, nf90_noerr, 1573, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % rmask,                            &
     &                     FORCES(ng) % srflx)
        IF (FoundError(status, nf90_noerr, 1620, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % vmask,                            &
     &                     FORCES(ng) % svstr)
        IF (FoundError(status, nf90_noerr, 1674, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % vmask,                            &
     &                     FORCES(ng) % bvstr)
        IF (FoundError(status, nf90_noerr, 1718, MyFile)) THEN
          IF (Master) THEN
...
!
      ...
      IF (FoundError(exit_flag, NoError, 2643, MyFile)) RETURN
