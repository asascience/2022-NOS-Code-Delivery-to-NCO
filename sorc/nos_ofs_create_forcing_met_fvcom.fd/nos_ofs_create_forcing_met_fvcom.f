...
!      integer,dimension(16,3) :: iparms  
      ...
      character*200 OFS,DBASE*10,DBASE_MET_NOW*10, OCEAN_MODEL*20
      character*200 header,BUFFER,GRIBFILE,GRIDFILE,OUTPUTFILE,FNAME
      character*200 OUTPUTFILE1,OUTPUTFILE2,OUTPUTFILE3
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      write(*,*)'write native grid coordinates of met forcing in the subdomain' 
      FNAME=trim(DBASE)//'_lonlat.dat'
      OPEN(99,file=trim(FNAME))
      DO J=1,JSUB
      DO I=1,ISUB
         lonsub(i,j)=lon(IMIN+I-1,JMIN+J-1)
         latsub(i,j)=lat(IMIN+I-1,JMIN+J-1)
         write(99,*)I,J,lonsub(i,j), latsub(i,j)
      ENDDO
      ENDDO    
      close(99)
      allocate(XINP(NNSUB) )
...
!              stop
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,0)
      ...
      ELSE
        FNAME='DPT.'//trim(DBASE)
        INQUIRE(FILE=TRIM(FNAME),EXIST=FEXIST)
        IF((FEXIST) .AND. (tdair_L .GT. 0.0))spfh_L = 1.
      ENDIF
...
!        print *,'TCDC_L=',tcdc_L
     ...
     1    lhtfl_L .GT. 0.0 .AND. shtfl_L .GT. 0.0 )nflux_L = 1.
      ...
      ITMP=pair_L+Uwind_L+Vwind_L+Tair_L+qair_L+dlwrf_L+dswrf_L	+spfh_L
      IF(ITMP .LE. 0)THEN
	...
	ENDIF
     ...
     &          tcdc_L,evp_L,prate_L,spfh_L,globalstr)
...
1000  NTMP0=-9999
	...
	IF(FEXIST .and. (.NOT. FOPEN))OPEN(81,file=trim(FNAME))
        IF(.NOT.FEXIST)THEN
            DO J=1,JSUB
	    DO I=1,ISUB
             pair(i,j)=101325.0  !! in Pa NDFD doesn't include air pressure so set it as a constant
	    ENDDO
	    ENDDO
        ELSE
...
!           print *,'test=',IND_DATE,IND_HOUR,IND_HOUR_END,
 !    1    BUFFER(IND_HOUR:IND_HOUR_END) 
     ...
     1    BUFFER(IND_DATE:IND_DATE+9)
          ...
          DO J=1,JSUB
          DO I=1,ISUB
            write(22,*)I,J,spfh(i,j)
          enddo
          enddo
        ELSE
          write(*,*)'Specific humility file does not exist !!'
          write(*,*)'calculate SPFH from DPT and air Temp'
C  calculate specific  humility from dew point temperature according to
C  formulas from GLERL, Ayumi (Arden L. Buck, 1981- equation 8)
          FNAME='DPT.'//trim(DBASE)
          INQUIRE(FILE=TRIM(FNAME),EXIST=FEXIST,OPENED=FOPEN)
          IF(FEXIST)THEN
            a0=6.1121
            ...
            DO J=1,JSUB
            DO I=1,ISUB
               ea=a0*(exp((a1*tdair(i,j))/(tdair(i,j)+a2)))*(a3+(a4*(pair(i,j)/100)))  !pair pa to mb
               tmp1(i,j)=(ea*a5)/(a6-a7*ea)
!               if (ea .lt. 10)then  !! AJ during summer time mostly ea > 10, so this if statement cause problems 06/09/2021  
!                   tmp1(i,j)=(ea*a5)/(a6-a7*ea)
!               else
!                   tmp1(i,j)=-99999.9
!               endif
...
!              if(OFS .eq. 'lmhofs') tmp1(i,j)=value*0.001 !! MP
              if(OFS .eq. 'lmhofs' .or. ofs .eq. 'lsofs' .or. ofs .eq. 'loofs' .or. ofs .eq. 'leofs') then
                 tmp1(i,j)=value*0.001
              endif
            ENDDO
	    ENDDO
            DO J=1,JSUB
...
!             call INTERP_REMESH(NNSUB,XINP,YINP,ZINP,
     ...
     *        NODEm,XOUT,YOUT,netswrfm,weightnodes,weights,1)
            ...
            IF (spfh_L .gt. 0.0)then
              ...
              DO I=1,ISUB
              DO J=1,JSUB
!                if (spfh(I,J) .GT. -999.0)then  !calculated spfh < 0.0 sometimes, which should not be used 
                if (spfh(I,J) .GT. 0.0)then
                  NTMP=NTMP+1
                  oned1(NTMP)=lonsub(i,j)
                  oned2(NTMP)=latsub(i,j)
                  oned3(NTMP)=spfh(i,j)
...
!!!!
	ENDIF 
        ...
        IF (IGRD .gt. 0) THEN
                ...
                I_prate=0.0
                I_spfh=0.0
                write(*,*)'calculating average','NODEm=',NODEm,'NELEm=',NELEm
                IF (TRIM(OCEAN_MODEL) .EQ. "FVCOM" )THEN
                  DO ITMP=1,NELEm
                   if (uwindm(ITMP) .GT. -999.0) then
                      sum_u=sum_u+uwindm(ITMP)
                      I_u=I_u+1
                   else
                      write(*,*)'uwindm less than -999 at element', ITMP,uwindm(ITMP)     
                      if (uwindm(ITMP-1) .GT. -999.0)uwindm(ITMP)=uwindm(ITMP-1)
                   endif
                   if (vwindm(ITMP) .GT. -999.0) then
                      sum_v=sum_v+vwindm(ITMP)
                      I_v=I_v+1
                   else 
                      write(*,*)'vwindm less than -999 at element',ITMP,vwindm(ITMP)   
                      if (vwindm(ITMP-1) .GT. -999.0)vwindm(ITMP)=vwindm(ITMP-1)      
                   endif
                  ENDDO
                  avg_u=0.0
                  avg_v=0.0                
                  if (i_u .gt. 0)avg_u=sum_u/i_u
                  if (i_v .gt. 0)avg_v=sum_v/i_v
                  WRITE(*,*)'aveu and v=',i_u,i_v,avg_u,avg_v
                  DO ITMP=1,NELEm
                    if (uwindm(ITMP) .LE. -999.0)uwindm(ITMP)=avg_u
                    if (vwindm(ITMP) .LE. -999.0)vwindm(ITMP)=avg_v
                  ENDDO
                ELSEIF (TRIM(OCEAN_MODEL) .EQ. "SELFE" )THEN
                  DO ITMP=1,NODEm
                   if (uwindm(ITMP) .GT. -999.0) then
                  ...
                  if (i_u .gt. 0)avg_u=sum_u/i_u
                  if (i_v .gt. 0)avg_v=sum_v/i_v
                  WRITE(*,*)'ave u and v=',i_u,i_v,avg_u,avg_v                  
                  DO ITMP=1,NODEm
                    if (uwindm(ITMP) .LE. -999.0)uwindm(ITMP)=avg_u
                    if (vwindm(ITMP) .LE. -999.0)vwindm(ITMP)=avg_v
                  ENDDO
                ...
                DO ITMP=1,NODEm
                   if (nfluxm(ITMP) .GT. -999.0) then
                      sum_flux=sum_flux+nfluxm(ITMP)
                      I_flux=I_flux+1
                   else
                      if (nfluxm(ITMP-1) .GT. -999.0)nfluxm(ITMP)=nfluxm(ITMP-1)     
                   endif
                   if (tairm(ITMP) .GT. -999.9)then
                      sum_tair=sum_tair+tairm(ITMP)
                      I_tair=i_tair+1
                   else
                      if (tairm(ITMP-1) .GT. -999.0)tairm(ITMP)=tairm(ITMP-1)     
                   endif
                   if (pairm(ITMP) .GT. -999.9)then
                      sum_pair=sum_pair+pairm(ITMP)
                      I_pair=i_pair+1
                   else
                      if (pairm(ITMP-1) .GT. -999.0)pairm(ITMP)=pairm(ITMP-1)     
                   endif                      
                   if (netswrfm(ITMP) .GT. -999.9)then
                      sum_nets=sum_nets+netswrfm(ITMP)
                      I_nets=i_nets+1
                   else
                      if (netswrfm(ITMP-1) .GT. -999.0)netswrfm(ITMP)=netswrfm(ITMP-1)      
                   endif
                   if (dlwrfm(ITMP) .GT. -999.9)then
                      sum_dl=sum_dl+dlwrfm(ITMP)
                      I_dl=i_dl+1
                   else
                      if (dlwrfm(ITMP-1) .GT. -999.0)dlwrfm(ITMP)=dlwrfm(ITMP-1)   
                   endif
                   if (qairm(ITMP) .GT. -999.9)then
                      sum_qair=sum_qair+qairm(ITMP)
                      I_qair=i_qair+1
                   else
                      if (qairm(ITMP-1) .GT. -999.0)qairm(ITMP)=qairm(ITMP-1)     
                   endif

                   if (tdairm(ITMP) .GT. -999.9)then
                     sum_tdair=sum_tdair+tdairm(ITMP)
                     I_tdair=i_tdair+1
                   else
                     if (tdairm(ITMP-1) .GT. -999.0)tdairm(ITMP)=tdairm(ITMP-1)      
                   endif
                   if (tcdcm(ITMP) .GT. -999.9)then
                     sum_tcdc=sum_tcdc+tcdcm(ITMP)
                     I_tcdc=i_tcdc+1
                   else
                     if (tcdcm(ITMP-1) .GT. -999.0)tcdcm(ITMP)=tcdcm(ITMP-1)      
                   endif

                   if (evpm(ITMP) .GT. -999.9)then
                     sum_evp=sum_evp+evpm(ITMP)
                     I_evp=i_evp+1
                   else
                     if (evpm(ITMP-1) .GT. -999.0)evpm(ITMP)=evpm(ITMP-1)      
                   endif
                   if (pratem(ITMP) .GT. -999.9)then
                      sum_prate=sum_prate+pratem(ITMP)
                      I_prate=i_prate+1
                   else
                      if (pratem(ITMP-1) .GT. -999.0)pratem(ITMP)=pratem(ITMP-1)     
                   endif
                   if (spfhm(ITMP) .GT. -999.9)then
                      sum_spfh=sum_spfh+spfhm(ITMP)
                      I_spfh=i_spfh+1
                   else
                      if (spfhm(ITMP-1) .GT. -999.0)spfhm(ITMP)=spfhm(ITMP-1)   
                   endif
                ENDDO   
                if (i_flux .gt. 0)avg_flux=sum_flux/i_flux
                if (i_tair .gt. 0)avg_tair=sum_tair/i_tair
                if (i_pair .gt. 0)avg_pair=sum_pair/i_pair
                if (i_nets .gt. 0)avg_nets=sum_nets/i_nets
                if (i_dl   .gt. 0)avg_dlwrf=sum_dl/i_dl
                if (i_qair .gt. 0)avg_qair=sum_qair/i_qair
                if (I_tdair .gt. 0)avg_tdair=sum_tdair/i_tdair
                if (I_tcdc .gt. 0)avg_tcdc=sum_tcdc/i_tcdc
                if (i_evp  .gt. 0)avg_evp=sum_evp/i_evp
                if (i_prate .gt. 0)avg_prate=sum_prate/i_prate
                if (I_spfh .gt. 0)avg_spfh=sum_spfh/i_spfh
                WRITE(*,*)i_flux,i_tair,i_pair,i_nets,i_dl,i_qair,I_tdair,I_tcdc,i_evp,i_prate,I_spfh
                WRITE(*,*)'avg_flux=',avg_flux,'avg_tair=',avg_tair,'avg_pair=',avg_pair
                WRITE(*,*)'avg_nets=',avg_nets,'avg_dlwrf=',avg_dlwrf,'avg_qair=',avg_qair
                WRITE(*,*)'avg_tdair=',avg_tdair,'avg_tcdc=',avg_tcdc,'avg_evp=',avg_evp,'avg_prate=',avg_prate
                DO ITMP=1,NODEm
                   if (nfluxm(ITMP) .LE. -999.0) nfluxm(ITMP)=avg_flux
                   if (Tairm(ITMP) .LE. -999.0) Tairm(ITMP)=avg_tair
                   if (pairm(ITMP) .LE. -999.0) pairm(ITMP)=avg_pair
                   if (netswrfm(ITMP) .LE. -999.0) netswrfm(ITMP)=avg_nets
                   if (dlwrfm(ITMP) .LE. -999.0) dlwrfm(ITMP)=avg_dl
                   if (qairm(ITMP) .LE. -999.0) qairm(ITMP)=avg_qair
                   if (tdairm(ITMP) .LE. -999.0) tdairm(ITMP)=avg_tdair
                   if (tcdcm(ITMP) .LE. -999.0) tcdcm(ITMP)=avg_tcdc
                   if (evpm(ITMP) .LE. -999.0) evpm(ITMP)=avg_evp
                   if (pratem(ITMP) .LE. -999.0) pratem(ITMP)=avg_prate
                   if (spfhm(ITMP) .LE. -999.0) spfhm(ITMP)=avg_spfh
                ENDDO   
!        IF (IGRD .GT. 0)THEN
...
810             FORMAT(I4.4,A1,I2.2,A1,I2.2,A1,I2.2,A1,I2.2,A10)
     ...
     &          netswrfm,dlwrfm,qairm,tdairm,tcdcm,evpm,pratem,
     &          spfhm,globalstr)
