...
!     &       -jbase_date
            day_start=day_start+jbase_date-JULIAN(yearb,monthb,dayb,hourb)
            base_date(1)=IYR
            base_date(2)=IMM
            base_date(3)=IDD
            base_date(4)=IHH
...
!      PRINT *,trim(VNAME),ocean_time,NTIMES
	...
	stop
      ELSEIF ( (day_start-day_hotrestart) .GT. 7.0 )THEN
        write(*,*)'restart file is too old'
	write(*,*)'initial time= ',day_hotrestart
	write(*,*)'current time= ',day_start
	write(*,*)'redefine restart time= day_start - 7'
        day_hotrestart=day_start-7.0  !allow to search 7 days backward
	CHANGE_TIME=.TRUE.
	COLD_START="T"
         ntimes=0
      ENDIF
...
100   FORMAT(I4.4,3I2.2,2x,I10,f12.4,2x,F12.4,a2)		     
