...
# Rename output files before continuing forecast to avoid overwriting
    ...
    if [ $NFILE -gt 0 ]; then
     ...
     if [ $NFILE -gt 0 ]; then
       latest_restart_f=`ls -trl  *${OFS}.rst.forecast*.nc | tail -1 | awk '{print $NF}' `
     fi
# Rename output files before continuing forecast to avoid overwritten
     NFILE=`ls -al *${OFS}.stations.forecast*.nc | wc -l`
     if [ $NFILE -gt 0 ]; then
       latest_station_f=`ls -trl  *${OFS}.stations.forecast*.nc | tail -1 | awk '{print $NF}' `
...
if [ ${OCEAN_MODEL} == "FVCOM" -o ${OCEAN_MODEL} == "fvcom" ]; then
  ...
  NFILE=`ls -al *${OFS}.stations.forecast*.nc | wc -l`
  if [ $NFILE -gt 0 ]; then
    latest_station_new=`ls -trl  *${OFS}.stations.forecast*.nc | tail -1 | awk '{print $NF}' `
    mv ${latest_station_new} ${latest_station_new}.new
  fi 
  NFILE=`ls -al *${OFS}.stations.forecast*.nc.old | wc -l`
  if [ $NFILE -gt 0 ]; then
    latest_station_f=`ls -trl  *${OFS}.stations.forecast*.nc.old | tail -1 | awk '{print $NF}' `
  fi 
