...
if [ -s ${FIXofs}/nos.${RUN}_z0_vary.nc ]; then
   cp -p ${FIXofs}/nos.${RUN}_z0_vary.nc $DATA/nos.${RUN}_z0_vary.nc
fi
...
#    NQCK_INTERVAL=`expr $NQCK_INTERVAL / 3600`
    ...
    if [ -f damee4_fwd_outer0_0001.nc ]; then
      mv damee4_fwd_outer0_0001.nc nos.${RUN}.fwd_outer0.$PDY.t${cyc}z.nc
      cp -p nos.${RUN}.fwd_outer0.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f damee4_fwd_outer1_0001.nc ]; then
      mv damee4_fwd_outer1_0001.nc nos.${RUN}.fwd_outer1.$PDY.t${cyc}z.nc
      cp -p nos.${RUN}.fwd_outer1.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f damee4_fwd_outer2_0001.nc ]; then
      mv damee4_fwd_outer2_0001.nc nos.${RUN}.fwd_outer2.$PDY.t${cyc}z.nc
      cp -p nos.${RUN}.fwd_outer2.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f ocean_mod.nc ]; then
      mv ocean_mod.nc nos.${RUN}.mod.$PDY.t${cyc}z.nc
      cp -p nos.${RUN}.mod.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
#   copy back the new improved initial conditions
    initfile=nos.${RUN}.init.nowcast.$PDY.t${cyc}z.nc
...
#######################
    ...
    NFILE=`ls -al nos.${OFS}.surface.nowcast*_????.nc | wc -l`
    if [ $NFILE -gt 0 ]; then
...
#######################
    NFILE=`ls -al *${OFS}.fields.forecast*.nc | wc -l`
    if [ $NFILE -gt 0 ]; then
      $USHnos/nos_ofs_rename.sh $OFS $OCEAN_MODEL $RUNTYPE 3d "$time_nowcastend" "$time_nowcastend"
    fi
    NFILE=`ls -al nos.${OFS}.surface.forecast*.nc | wc -l`
    if [ $NFILE -gt 0 ]; then
