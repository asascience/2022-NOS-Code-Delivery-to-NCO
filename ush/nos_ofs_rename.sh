...
###############################################################################
#cd /gpfs/dell1/ptmp/Aijun.Zhang/tbofs/work/nos_tbofs_nf_12_dev
#cd /gpfs/dell1/ptmp/Aijun.Zhang/sfbofs/work/nos_sfbofs_nf_09_dev
...
if [ $RUNTYPE == "NOWCAST" -o $RUNTYPE == "nowcast" ]; then 
  if [ $FILETYPE == '2d' -o $FILETYPE == '2D' ]; then
    filehead=nos.${OFS}.2ds.n
  elif [ $FILETYPE == '3d' -o $FILETYPE == '3D' ]; then
    filehead=nos.${OFS}.fields.n
  fi
elif [ $RUNTYPE == "FORECAST" -o $RUNTYPE == "forecast" ]; then
  if [ $FILETYPE == '2d' -o $FILETYPE == '2D' ]; then
    filehead=nos.${OFS}.2ds.f
  elif [ $FILETYPE == '3d' -o $FILETYPE == '3D' ]; then
    filehead=nos.${OFS}.fields.f
  fi
fi
filetail=$day'.t'$cyc'z.nc'
...
if [ ${OCEAN_MODEL} == "FVCOM" -o ${OCEAN_MODEL} == "fvcom" ]; then
  ...
  if [ $FILETYPE == '2d' -o $FILETYPE == '2D' ]; then
     NFILE=`ls -al *${OFS}.surface.${RUNTYPE}.$day.t${cyc}z_????.nc | wc -l`
     if [ $NFILE -gt 0 ]; then
       echo $NFILE > tmp.out
       ls -al *${OFS}.surface.${RUNTYPE}.$day.t${cyc}z_????.nc  | awk '{print $NF}' >> tmp.out
     fi

  elif [ $FILETYPE == '3d' -o $FILETYPE == '3D' ]; then
     NFILE=`ls -al *${OFS}.fields.${RUNTYPE}.$day.t${cyc}z_????.nc | wc -l`
     if [ $NFILE -gt 0 ]; then
       echo $NFILE > tmp.out
       ls -al *${OFS}.fields.${RUNTYPE}.$day.t${cyc}z_????.nc  | awk '{print $NF}' >> tmp.out
     fi
  fi
fi
