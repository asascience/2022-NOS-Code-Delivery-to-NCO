# control files for WCOFS4_DA, which is read in by shell script 
...
export RUNTIME_CTL_FOR=nos.${OFS}.roms.for.in
...
export MODELTIDE_HC=nos.${OFS}.modeltide.HC.nc
export MODELTIDE=nos.${OFS}.modeltide.nc
########################################################
...
export NTILE_I=8   # Should match NtileI in nos.$OFS.roms.in
export NTILE_J=16  # Should match NtileJ in nos.$OFS.roms.in
...
#              It must be equal or greater than one.
#### 	FOR DATA ASSIMILATION (ONLY WITH WCOFS FOR NOW)
#  ERR_TEMP    Temperature observation error standard deviation
#  ERR_V       Velocity (HF Radar) observation error standard deviation
#  ERR_SSH     SSH observation error standard deviation
#  EPSDOP      DOP cut off value for HF radar QC
#  HF_HMIN     Depth cut off value for HF radar QC
#  MODELTIDE_HC Model tidal water level harmonics constants on model grid for SSH assimilation
#  MODELTIDE   Same as above but already has nodal factor and equilibrium arguments applied
